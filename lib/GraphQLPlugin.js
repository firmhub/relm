...
var parse = interopDefault(parser);
...
var GraphQLPlugin = function () {
  function GraphQLPlugin(createQuery) {
    classCallCheck(this, GraphQLPlugin);

    this.createQuery = createQuery;
  }

  createClass(GraphQLPlugin, [{
    key: 'apply',
    value: function apply(component, source) {
      // Get the child fragments - saved for access from parent compoennts
      component.fragments = _.mapValues(component.components, function (it) {
        return it.fragments || {};
      });
      ...
      // Make query creators
      component.queries = _.mapValues(queries, function (query, name) {
...
function tag(literals) {
  ...
  var source = _.reduce(substitutions, joinString, '') + _.last(literals);
